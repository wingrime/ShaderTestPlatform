...
vec3 rvec = normalize(vec3(rnd(), rnd(), rnd()));
...
vec3 rayTrace(vec3 ro_in, vec3 rd_in) {
    ...
    for (int i = 0; i < RAY_D ; i++) {
        ...
        float intense =0.0;
                ...
                if (rc.mat.idx == G_MAT)
                    col = vec3(0.0,1.0,0.0)* lambert(rc.n,light_dir)*shadow_m*sign(float(i));
                else if (rc.mat.idx == D_MAT)
                    col = vec3(0.5)* lambert(rc.n,light_dir)*shadow_m*sign(float(i));
                else{
...
seed = hash(1u+hash(  hash(floatBitsToUint(uv.x))+hash(hash(floatBitsToUint(uv.y))+hash(floatBitsToUint(time)))  ));
    ...
    s[2] = sphere_t((vec3(-0.2*sin(2.0*time),-0.1*cos(2.0*time),-0.8*sin(2.0*time))), 0.25,g_mat);
