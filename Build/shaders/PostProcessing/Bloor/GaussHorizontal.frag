...
float bS = blurSize/10.0;
  ...
  vec4 sum = vec4(0.0);
  sum += texture2D(texFB, vec2(c.x , c.y- 4.0*bS)) * 0.05;
  sum += texture2D(texFB, vec2(c.x, c.y - 3.0*bS)) * 0.09;
  sum += texture2D(texFB, vec2(c.x , c.y- 2.0*bS)) * 0.12;
  sum += texture2D(texFB, vec2(c.x , c.y- bS)) * 0.15;
